...
export default function AboutPage() {
  const { data: teamData, isLoading, error } = useGetTeamQuery({});

  const normalizeKeywords = (keyword: unknown): string[] => {
    if (!keyword) return [];
    if (Array.isArray(keyword)) {
      const flat = keyword.flatMap((k) => {
        if (typeof k === 'string') {
      ...
      return flat.filter((k): k is string => typeof k === 'string' && k.length > 0);
    }
            ...
            <div className="mt-14">
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                  {teamData.data.map((member: TeamMember, idx: number) => (
                    <article key={`${member._id || member.id}-${idx}`} className="bg-white shadow-sm ring-1 ring-black/5 transition-all duration-300 hover:scale-105">
                        ...
                        <img
                          src={
                            member.profilePicture && member.profilePicture.trim() !== ''
                              ...
                              : member.image || member.imageUrl || "/window.svg"
                          }
                          alt={member.name}
                      ...
                      <div className="px-6 pb-7 pt-6">
                        <h3 className="font-serif text-lg font-semibold text-neutral-900">{member.name}</h3>
                        <p className="mt-1 text-xs text-neutral-500">
                          {member.designation || member.role || member.position}
                        </p>
                        ...
                        {(() => {
                          const badges =
                            member.badges && member.badges.length > 0
                              ? member.badges
                              : normalizeKeywords(member.keyword);
                ...
                <div className="text-center py-12">
                  <p className="text-neutral-600">No team members available at the moment.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <section className="bg-[#2c2c2c] py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-12 space-y-4">
              {clientExperience.map(({ title, description, Icon }: { title: string; description: string; Icon: any }) => (
